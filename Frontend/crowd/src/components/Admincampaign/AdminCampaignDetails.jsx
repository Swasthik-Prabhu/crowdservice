...
const AdminCampaignDetails = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://127.0.0.1:8000/campaigns/${campaign_id}/donors`)
      .then((response) => setDonors(response.data || [])) // Ensure donors is an empty array if response is null or undefined
      .catch((error) => {
        console.error('Error fetching donors:', error);
        setDonors([]); // Set donors to an empty array on error
      });
  }, [campaign_id]);
